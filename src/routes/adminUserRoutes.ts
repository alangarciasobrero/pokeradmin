...
router.get('/:id/ledger', requireAdmin, async (req: Request, res: Response) => {
  try {
    ...
    // Calcular totales usando la misma lógica que PaymentRepository
    // Balance = suma de (paid_amount - amount) para cada payment (sin settlements)
    // Si es negativo = debe dinero, si es positivo = tiene crédito
    let totalAmount = 0;  // Total que debe (inscripciones + cash)
    let totalPaidAmount = 0;  // Total que ha pagado
    
    for (const p of payments) {
      const pAny = p as any;
      // Excluir settlements del cálculo (son solo audit trail)
      if (pAny.source === 'settlement') continue;
      
      const amt = Number(pAny.amount || 0);
      const paid = Number(pAny.paid_amount || 0);
      
      totalAmount += amt;
      totalPaidAmount += paid;
    }

    const balance = totalPaidAmount - totalAmount;

    res.render('admin_user_ledger', { 
      user, 
      payments, 
      registrations, 
      cashParts, 
      totals: { 
        ...
        totalRegistrationsCost: totalAmount,  // Renombrado pero ahora es el total de amount en payments
        balance 
      } 
    });
  } catch (e) {
...
// POST /admin/users/:id/payments/bulk-pay - pago m�ltiple
router.post('/:id/payments/bulk-pay', requireAdmin, async (req: Request, res: Response) => {
  try {
    ...
    if (!paymentIds.length) {
      if (req.session) req.session.flash = { type: 'error', message: 'No seleccionaste ning�n movimiento' };
    ...
    if (req.session) req.session.flash = { type: 'error', message: 'Error al procesar el pago m�ltiple' };
    return res.redirect(`/admin/users/${req.params.id}/ledger`);
  }
});


// POST /admin/users/:id/payments/quick-pay - pago r�pido desde lista de deudores
  ...
  try {
    ...
    const payments = await Payment.findAll({
      where: {
        ...
        [Op.or]: [{ paid: false }, sequelize.literal('amount > COALESCE(paid_amount, 0)')]
      },
      order: [['payment_date', 'ASC']]
    });
    ...
    for (const payment of payments) {
      ...
      await Payment.create({
        user_id: userId, amount: amountToApply, payment_date: now, source: 'settlement',
        reference_id: pAny.id, paid: true, paid_amount: amountToApply,
        method: methodWithActor, recorded_by_name: recordedByName
      });
    ...
    if (req.session) {
      req.session.flash = {
        type: 'success',
        message: `Pago registrado: `import { Router, Request, Response } from 'express';
import { requireAdmin } from '../middleware/requireAuth';
import { User } from '../models/User';
import Payment from '../models/Payment';
import { Registration } from '../models/Registration';
...
`${totalApplied.toFixed(0)} aplicado a ${paidPayments.length} movimiento(s)${remainingAmount > 0 ? `. Sobrante: `import { Router, Request, Response } from 'express';
import { requireAdmin } from '../middleware/requireAuth';
import { User } from '../models/User';
import Payment from '../models/Payment';
import { Registration } from '../models/Registration';
...
`${remainingAmount.toFixed(0)}` : ''}`
    ...
    if (req.session) req.session.flash = { type: 'error', message: 'Error al procesar el pago r�pido' };
    return res.redirect('/admin/debtors');
  }
});
